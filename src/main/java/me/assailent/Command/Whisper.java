...
public class Whisper extends AbstractPlayerCommand {

    private final WorldbuildersChat plugin;

    public Whisper(WorldbuildersChat plugin) {
        super("Whisper", "Whisper so less people can hear you!");
        this.plugin = plugin;
    }
    ...
    protected void execute(@NonNullDecl CommandContext commandContext, @NonNullDecl Store<EntityStore> store, @NonNullDecl Ref<EntityStore> ref, @NonNullDecl PlayerRef playerRef, @NonNullDecl World world) {
        ...
        playerRef.sendMessage(formatting.message(formatting.format(config.getStartWhispering(), map)));

        world.execute(() -> {
            WorldbuildComponent worldbuildComponent = store.ensureAndGetComponent(playerRef.getReference(), plugin.getWorldbuildComponent());
            worldbuildComponent.whisper = true;
        });
    }
}
